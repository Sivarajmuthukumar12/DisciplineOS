...
function Habits({ user }) {
  ...
  const fetchHabits = async () => {
    try {
      const r = await axios.get(`sivaraj12km.pythonanywhere.com/habits/${user.id}`);
      setHabits(r.data);
    } catch {}
  };
  ...
  const addHabit = async () => {
    ...
    await axios.post("sivaraj12km.pythonanywhere.com/habits", { user_id: user.id, name: habitName, category });
    setHabitName("");
    showMsg("Habit added! 🎉");
    fetchHabits();
  };
  ...
  const completeHabit = async (id) => {
    await axios.post("sivaraj12km.pythonanywhere.com/habits/complete", { habit_id: id });
    showMsg("Habit completed! 🔥 Streak increased!");
    fetchHabits();
  };

  const deleteHabit = async (id) => {
    await axios.delete(`sivaraj12km.pythonanywhere.com/habits/${id}`);
